sap.ui.define([
    ...
    return BaseController.extend("com.cy.rbm.controller.RebateAgreementDetails", {
        onInit: function () {
...
onSave(){
    ...
    onSuspendAggrement(){
      MessageToast.show("Agreement Suspended");
    },
    onDeleteRule(oEvent) {
      var oModel = this.getOwnerComponent().getModel("createAgreementModel");
    ...
    oneditAggrement: function () {
      ...
      this.getOwnerComponent().getModel("createAgreementModel").setData({
        ...
        rules: [{
          active: true,
          ruleId: "R-2025-001",
          ruleName: "Annual Beverage Volume Incentive",
          ruleType: "TIERED",
          calculationBasis: "QTY",      // Quantity based
          calcMethod: "FIXED_UNIT",     // Fixed amount per unit
          tierApp: 0,                   // 0 = Retroactive (All units at highest tier)

          tiers: [
            { from: 0, to: 1000, rate: 10, bonus: 0 },
            { from: 1001, to: 5000, rate: 12, bonus: 500 },
            { from: 5001, to: 999999, rate: 15, bonus: 2000 }
          ],

          selectedInclusions: ["BEV", "COLA", "JUICE"],
          selectedExclusions: ["ALC", "RET"], // Exclude Alcohol and Returns

          conditions: {
            minTransaction: 100,      // Minimum 100 qty per invoice to count
            minPeriodVolume: 500,     // Minimum 500 qty total to qualify
            maxRebateAmount: 500000,  // Cap at 500k
            maxRate: null,
            validityDate: null,       // Or a Date object
            paymentTerms: "30",       // Within 30 days
            excludeDiscounts: "ALL"   // Exclude all discounts from basis
          },

          rateSummary: "Tiered: ₹10 - ₹15 / Unit" // Summary string for list display
        }],
        // 3️⃣ Tiers (Initialize with one empty row for better UX)
        tiers: [
          { from: 0, to: "", rate: "", bonus: "" }
        ],
      ...
      this.getOwnerComponent().getModel("createAgreementModel").setData(this.temp);
